...
// TODO: 여기서부터 작업시작. git revert랑 reset의 차이를 알고서 정리. 
## 4. Git(로컬 저장소) 되돌리기  
Git의 로컬 저장소에서 변경 사항을 되돌리는 과정. 즉, GitHub(원격 저장소)에 반영되기 전에 로컬 작업을 수정. 

| 단계           | 명령어 | 설명 |
|--------------|--------------------------------------|----------------------------------|
| **커밋 되돌리기** | [`git revert <커밋해시>`](#revert) | 지정한 커밋을 되돌리고 새로운 커밋을 생성 |
...
```
- 기존 커밋을 되돌리는 새로운 커밋을 생성하는 명령어
- 이전 변경 사항을 "취소"하는 새로운 커밋을 만들고, 기존 커밋 히스토리는 유지
...
git reset --soft <커밋해시>
```
- 특정 커밋으로 이동하되, 변경 사항은 유지
- 마지막 커밋 메시지를 수정하고 싶거나, 여러 개의 커밋을 하나로 합치고 싶을 때 사용
...
git reset --hard <커밋해시>
```
- 특정 커밋으로 이동하고, 이후 변경 사항까지 모두 삭제
- 수정한 내용이 전부 삭제됨 (복구 불가)
<br/>
...
| 단계           | 명령어 | 설명 |
|--------------|---------------------------------|----------------------------------|
| **브랜치 목록** | [`git branch`](#branch) | 현재 존재하는 브랜치 목록을 확인합니다. |
| **브랜치 생성** | [`git branch <브랜치명>`](#branch-create) | 새 브랜치를 생성합니다. |
| **브랜치 이동** | [`git switch <브랜치명>`](#switch) | 다른 브랜치로 이동합니다. |
| **브랜치 병합** | [`git merge <브랜치명>`](#merge) | 현재 브랜치에 다른 브랜치를 병합합니다. |
| **히스토리 정리** | [`git rebase <브랜치명>`](#rebase) | 병합 대신 커밋을 다시 적용하여 정리합니다. |
